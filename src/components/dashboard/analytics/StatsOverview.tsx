    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
      ...
      <div className="col-span-1 flex flex-col gap-4">
        {statItems.map((item, idx) => (
          <div 
            key={idx} 
            className={cn(
              "relative overflow-hidden bg-gradient-to-br from-zinc-900/95 to-zinc-900/70 backdrop-blur-2xl border border-zinc-800 shadow-xl p-5 rounded-2xl transition-all duration-300 group",
              item.hover,
              item.border
            )}
          >
            ...
            <div className="flex items-center justify-between mb-4 px-1">
              <div className="flex items-center gap-2.5">
                <div className={cn("w-1 h-1 rounded-full animate-pulse shadow-[0_0_8px_rgba(255,255,255,0.4)]", item.dot)} />
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">{item.label}</span>
              </div>
              <div className={cn("p-1.5 rounded-xl border border-white/5 shadow-inner transition-transform group-hover:scale-110", item.bg, `text-${item.color}`)}>
                <item.icon className="w-3.5 h-3.5" />
              </div>
            </div>

            {/* Metric Value (Perfectly Aligned) */}
            <div className="relative px-1">
              <div className="text-3xl font-black text-white tracking-tighter leading-none relative z-10 transition-transform group-hover:translate-x-1">
                {item.value}
              </div>
              <div className={cn(
                "absolute inset-0 blur-2xl opacity-0 group-hover:opacity-10 transition-opacity",
                item.dot
              )} />
            </div>

            {/* Decorative Element */}
            <div className="absolute top-0 right-0 w-24 h-24 bg-white/[0.02] blur-xl rounded-full -mr-12 -mt-12" />
          </div>
        ))}
      </div>
      ...
      <div className="col-span-1 md:col-span-3 bg-zinc-900/50 backdrop-blur-md border border-zinc-800/50 p-6 rounded-3xl shadow-2xl flex flex-col group/chart overflow-hidden relative">
        ...
        <div className="flex items-center justify-between mb-8 relative z-10 px-2">
          <div>
            <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-zinc-500 mb-1 flex items-center gap-3">
        ...
        <div className="flex-1 min-h-[180px] relative z-10 px-2">
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 20, left: -25, bottom: 20 }}>
                <defs>
